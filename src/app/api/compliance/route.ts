...
export async function POST(req: Request) {
  ...
  if (body.type === "delete_reminder") {
    await prisma.businessReminder.update({ where: { id: body.id, userId }, data: { isActive: false } });
    return NextResponse.json({ ok: true });
  }

  if (body.type === "paid_reminder") {
    await prisma.businessReminder.update({ where: { id: body.id, userId }, data: { lastPaidDate: new Date() } });
    return NextResponse.json({ ok: true });
  }
