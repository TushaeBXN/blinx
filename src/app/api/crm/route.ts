...
export async function POST(req: Request) {
  ...
  if (body.action === "add_interaction") {
    ...
    // IDOR guard: verify contact belongs to this user
  ...
  if (body.action === "delete_interaction") {
    // IDOR guard: verify interaction belongs to a contact owned by this user
    const interaction = await prisma.contactInteraction.findUnique({
      where: { id: body.id },
      include: { contact: { select: { userId: true } } },
    });
    if (!interaction || interaction.contact.userId !== userId) {
      return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }
