...
import { runImpactAnalystAgent } from "@/lib/agents/impactAnalystAgent";
...
export async function runNightlyLoop(orgName = "Blinx Tech Education Nonprofit"): Promise<void> {
  ...
  if (memoryOnline) {
    ...
    for (const [id, desc] of [
      ...
      ["impactAnalystAgent", "Impact tracking and quarterly reports"],
    ] as [string, string][]) {
...
export async function initScheduler(): Promise<void> {
  ...
  // Impact Analyst — every Monday at 6:00 AM (scores new feedback)
  activeCrons.push(cron.schedule("0 6 * * 1", cronJob("impact-analyst", () => runImpactAnalystAgent().then(() => {}))));
      ...
      <body style="margin:0;padding:0;background:#F5F5F7;font-family:-apple-system,BlinkMacSystemFont,'Helvetica Neue',sans-serif">
        <div style="max-width:560px;margin:32px auto">
          ...
          <div style="background:#1D1D1F;border-radius:16px 16px 0 0;padding:28px 32px">
            ...
            <p style="margin:0;font-size:13px;color:#8E8E93">${month} · auto-generated by Blinx</p>
          </div>
