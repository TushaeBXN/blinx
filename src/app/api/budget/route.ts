...
export async function POST(req: Request) {
  ...
  if (body.action === "upsert_line") {
    ...
    if (id) {
      // IDOR guard: verify the line item belongs to this user's budget
      const existing = await prisma.budgetLineItem.findUnique({ where: { id }, include: { budget: { select: { userId: true } } } });
      if (!existing || existing.budget.userId !== userId) {
  ...
  if (body.action === "delete_line") {
    // IDOR guard: verify line item belongs to this user's budget
    const existing = await prisma.budgetLineItem.findUnique({ where: { id: body.id }, include: { budget: { select: { userId: true } } } });
    if (!existing || existing.budget.userId !== userId) {
      return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }
